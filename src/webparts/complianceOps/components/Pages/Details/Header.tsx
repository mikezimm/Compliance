...
const DetailsPageHook: React.FC<IDetailsPageProps> = ( props ) => {
 ...
 */
  ...
  const MainContent: JSX.Element = <div style={{ cursor: 'default' }}>
    <ul>
      <li>This tab will analyize the current site or library from a Records label standpoint.</li>
      <li>The goal is to fetch a sampling of items from a library and then see what labels are applied.</li>
      <li>It is not a simple fetch but has to look at labels on individual items and build a summary.</li>
  ...
  const TeachMe = teachBubble === null ? null : makeBubbleElementFromBubbles( lastBubble, getTeachBubbles( AllTeachBubbles ,'', 'Details' ), updateTour, closeTour );

  const DetailsPageElement: JSX.Element = mainPivotKey !== 'Details' ? null : <div className = { styles.page } style={ null }>
    { InfoElement}
    {/* <div id={ 'ComplDetailsStartTour' } ><Icon iconName={ 'MapPin' }/></div> */}
    { TeachMe }
  </div>;

  return ( DetailsPageElement );

}

export default DetailsPageHook;